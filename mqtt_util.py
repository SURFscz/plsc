...
def update_service(src, dst, config, sid):
    ...
    print("Removing stale service branches")
...
def update_collaboration(src, dst, _, cid):
...
def clean_collaboration(src, dst, cid):
    ...
    for service, cos in services.items():
        ...
        grp_dns = dst.rfind(f"ou=Groups,o={co_identifier},dc=ordered,dc={service}", "(&(objectClass=sczGroup))")
...
def delete_collaboration(dst, cid):
    service_dns = dst.find(dst.basedn, "(&(objectClass=dcObject)(ObjectClass=organization))",
...
def create_service_user(src, dst, _, cid, uid, service):
...
def create_service_user_groups(src, dst, cid, user, service):
    ...
    for grp_id, grp_name in groups.items():
        ...
        if len(grp_dns) == 1:
            old_dn, old_entry = list(grp_dns.items())[0]
            grp_entry = copy.deepcopy(old_entry)
            scz_members = old_entry.get('sczMember', [])
            if user_dn not in scz_members:
...
def remove_group_users(src, dst, _, cid, gid):
